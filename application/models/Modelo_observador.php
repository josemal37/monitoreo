...
/**
 * Description of Modelo_observador
 *
 * @author Jose
 */
class Modelo_observador extends CI_Model {
    ...
    public function get_id_proyecto_global() {
        try {
            ...
            if(!$query) {
                ...
                if($query->num_rows() == 0) {
                    ...
                    return $query->row()->id_proyecto_global;
                }
            }
        } catch (Exception $ex) {
            redirect(base_url() . 'observador/error');
    ...
    public function get_id_poa_gestion_actual() {
        try {
            $id_institucion = $this->session->userdata('id_institucion');
            $sql = "SELECT
                        PROYECTO.id_proyecto
